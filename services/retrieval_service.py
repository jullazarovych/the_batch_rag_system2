...
try:
    ...
    print(f"(Retriever) Error connecting to Weaviate (make sure 'docker compose up' is running): {e}")
...
def search_text_chunks(query: str, limit: int = 5, alpha: float = 0.7) -> list:
    ...
    try:
        ...
        print(f"(Retriever) Hybrid text search error: {e}")
        return []

def _get_clip_text_vector(text_query: str) -> list[float]:
    if not clip_model:
...
def search_images_by_text(query: str, limit: int = 3) -> list:
    ...
    if not clip_model:
        raise ConnectionError("CLIP model not available for image search.")

    try:
        query_vector = _get_clip_text_vector(query)
